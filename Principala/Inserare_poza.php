...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_FILES['fisier']) || $_FILES['fisier']['error'] !== UPLOAD_ERR_OK) {
        $errors['fisier'] = 'Trebuie selectata o poza.';
    } else {
        $ext = strtolower(pathinfo($_FILES['fisier']['name'], PATHINFO_EXTENSION));
        $allowed = ['png','jpg','jpeg','gif','webp','bmp'];
        if (!in_array($ext,$allowed)) { $errors['fisier']='Formate acceptate: PNG, JPG, JPEG, GIF, WEBP, BMP.'; }
        else {
    ...
    if (empty($errors)) {
        ...
        $stmt = $con->prepare("INSERT INTO poze (Link) VALUES (?)");
        $stmt->bind_param("s", $linkPath);
...
<html lang="ro">
    <head>
        <meta charset="UTF-8">
        <title>Inserare Poza</title>
    <?php include 'form_style.php'; ?>
    </head>
<body>
<div class="container">
    <a href="Principala.php" class="back">← Inapoi</a>
    <h2>🖼️ Inserare Poza</h2>
    <form method="post" enctype="multipart/form-data">
        <label>Selectati imaginea de pe calculator</label>

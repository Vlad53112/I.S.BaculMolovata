...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['fisier']) && $_FILES['fisier']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($ext,$allowed)) { $errors['fisier']='Tip nepermis (PDF, DOC, DOCX, XLS, XLSX, TXT).'; }
        else {
    ...
    if (empty($errors)) {
        ...
        $stmt = $con->prepare("INSERT INTO actenormative (data,Titlu, Link) VALUES (?,?,?)");
